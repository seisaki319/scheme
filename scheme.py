...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        return scheme_apply(func, operands, env)
        # END PROBLEM 5
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('Wrong # of arguments fam lol')
        # END PROBLEM 4
...
def do_and_form(expressions, env):
    ...
    tail = False
    while target is not False and expressions.second:
        expressions = expressions.second
        if not expressions.second:
            tail = True
        target = scheme_eval(expressions.first, env, tail)
    return target
    # END PROBLEM 13
...
def do_or_form(expressions, env):
    ...
    target = scheme_eval(expressions.first, env)
    tail = False
    while target is False and expressions.second:
        expressions = expressions.second
        if not expressions.second:
            tail = True
        target = scheme_eval(expressions.first, env, tail)
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            # BEGIN Extra Credit
            func = scheme_optimized_eval(first, env)
            operands = rest.map(lambda x: scheme_optimized_eval(x, env))
            result = scheme_apply(func, operands, env)
            # END Extra Credit
    return result
