...
; expect cadr
(define (squares x)
  (cons-stream (* x x) (squares (+ x 1)))
)
(car (squares 2))
; expect 4
(car (cdr-stream (squares 9)))
;expect 100
(car (cdr-stream (squares 2)))
; expect 9
...
(define (fib n)
(define (fib-helper past1 past2 k)
(if (= k 1) past2 (fib-helper past2 (+ past1 past2) (- k 1)))
)

(if (= n 0) 1 (fib-helper 1 1 n))
)
(fib 19)
; expect 6765
(fib 0)
...
(define (count-digits n)
  (define (f n cnt)
    (if (= n 0)
      cnt
      (f (quotient n 10) (+ cnt 1))
    )
  )
  (f n 0)
)
(count-digits (fib 1000))
; expect 209

;;;note that i edited the scheme primitive quotient function to do integer division instead of doing division and converting to int
(define (tail-fact n)
(define (fact-helper so-far k)
(if (= k 0) so-far (fact-helper (* k so-far) (- k 1)))
)
(fact-helper 1 n)
)
(tail-fact 3)
;expect 6
(count-digits (tail-fact 30))
;expect 33

(define (pow base n)
(define (pow-helper so-far k)
(if
(= k 0) so-far
(pow-helper (* base so-far) (- k 1)))
)
(pow-helper 1 n)
)
(pow 2 3)
...
(pow 1 2)
; expect 1
(count-digits (pow 5 15))
; expect 11
(count-digits (pow 2 350))
; expect 106


(define times-i-cry 'every-time)
times-i-cry
; expect every-time
...
(define (sesame-street)
(let ((c 'cookie) (cookie 'me)) (if (eq? cookie 'cookie) 'oatmeal-raisin 'choc-chip))
)
(sesame-street)
; expect choc-chip
